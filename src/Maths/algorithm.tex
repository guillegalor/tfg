...
\begin{theorem}
    ...
    \end{equation}
    donde $u_{I}, v_{I}$ y $r_{I}$ son los coeficientes de Bezout devueltos por el Algoritmo Euclídeo Extendido a la derecha con entrada $x^{2\tau}$ y \(S\), e \(I\) es el índice determinado por las condiciones \(\deg r_{I-1} \ge \tau\) y \(\deg r_{I} < \tau\). En particular, \(\lambda = v_{I}g\) y \(\omega = r_{I}g\) para algún \(g \in R\).
\end{theorem}
