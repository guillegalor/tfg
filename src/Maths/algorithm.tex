...
% Para demostrar la otra implicación realizaremos una prueba más general de lo necesaria para este resultado, pero que no será útil más adelante.
% La otra implicación es un caso específico de la prueba que realizaremos más adelante en el corolario~\ref{cor:llcm_n}, así que por evitar la repetición no la incluimos aquí.

% Vamos a ver que \(x - \sigma^{k}(\beta)\) divide a la derecha a \(x^{n} - 1\) para todo \(0 \le k \le n-1\), siendo el caso \(k = 0\) el que nos interesa para este resultado. Utilizando la definición de norma j-ésima, tenemos que:
%     \[
%     \begin{aligned}
%     N_i(\sigma^{k}(\beta)) &= \sigma^{k}(N_i(\beta)) = \sigma^{k}\left(c^{-1}\sigma(c)\sigma(c^{-1})\sigma^{2}(c)\sigma^{2}(c^{-1}) \ldots \sigma^{i}(c) \right) \\
%        &= \sigma^{k}(c)^{-1} \sigma^{k+i}(c)
%     \end{aligned}
%     .\]
%    Entonces, \(N_n(\sigma^{k}(\beta)) = \sigma^{k}(c)^{-1} \sigma^{k+n}(c) = \sigma^{k}(cc^{-1}) = 1\), y por el lema~\ref{lem:eval}, el resto de la división a la izquierda de \(x^{n} - 1\) por \(x - \sigma^{k}(\beta)\) es
%     \[
%     N_n(\sigma^{k}(\beta)) - N_0(\sigma^{k}(\beta)) = 1 - 1 = 0
%     .\]

%    Por tanto,\(x - \sigma^{k}(\beta)\) divide a la derecha a \(x^{n} - 1\) para todo \(0 \le k \le n-1\), finalizando la prueba.
\end{proof}
